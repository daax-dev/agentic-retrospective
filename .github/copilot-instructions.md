...
## Code Conventions
- Bash scripts: telemetry hook scripts must not break the session (avoid `set -e`; handle missing tools/IO errors by exiting 0). For non-hook scripts, prefer fail-fast `set -euo pipefail`. Quote expansions, no `eval`. Keep hook scripts fast (5s timeout) and side-effect-only (telemetry logging).
