...
- Package manager: pnpm (canonical, `pnpm-lock.yaml`, lockfile v9; CI uses `pnpm install --frozen-lockfile`). A stale `package-lock.json` also exists — do NOT use npm to install/update; do not commit npm lockfile changes.
...
- CI: GitHub Actions, single workflow `.github/workflows/publish.yml` — runs ONLY on push to `main` touching `package.json`. CI does NOT run on PRs; validate locally.
...
- ESLint (`.eslintrc.json`): `eslint:recommended` + `@typescript-eslint/recommended`. `@typescript-eslint/no-explicit-any` is `warn` (avoid `any`; justify when unavoidable). Unused vars are an error unless prefixed `_`. Run `pnpm run lint`.
