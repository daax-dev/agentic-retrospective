...
export class RetroRunner {
  ...
  private async validateEnvironment(): Promise<{ valid: boolean; error?: string }> {
    ...
    try {
      ...
      if (!isGit) {
        return {
          valid: false,
          error: 'Not a git repository. The /retro command requires git history to analyze.',
        };
      }
      return { valid: true };
    } catch {
  ...
  private generateActionItems(): ActionItem[] {
    ...
    for (const gap of this.gaps) {
      items.push({
        ...
        owner: null,
        success_metric: 'Data source available in next retro',
        effort: 2,
        impact: 3,
        risk_reduction: 3,
      });
  ...
  private async writeOutputs(report: RetroReport, evidenceMap: EvidenceMap): Promise<string> {
    ...
    if (!this.options.jsonOnly) {
      ...
      writeFileSync(join(outputPath, 'retro.md'), markdown);
    }
