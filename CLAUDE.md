...
- `pnpm` is the canonical package manager (CI uses `pnpm install --frozen-lockfile`; `pnpm-lock.yaml` is authoritative). A stale `package-lock.json` also exists — do not use npm to install or update deps.
- Publishing is release-driven via `.github/workflows/publish.yml` (runs on push to `main` touching `package.json`). CI does NOT run on pull requests — validate locally with `pnpm run validate` before opening a PR.
