...
- Style: `set -euo pipefail` in every script. Quote all expansions. No `eval`. Hook scripts must stay fast (5s timeout in `hooks.json`) and side-effect-only (append telemetry to `.logs/`).
