...
ClaudeSubagent {
  agentId, agentType,              // from subagents/agent-<id>.meta.json; agentType always present
  description?,                    // optional (sidecar fields vary; inventory §3.1)
  spawnedByToolUseId?,             // optional — links to parent Agent/Task tool_use; absent => lineage unknown
  usageTotals                      // summed (deduped) from subagents/agent-<id>.jsonl
}
...
**Dedup key:** `message.id`, keep the last record (streamed partials share `message.id` but each has a
distinct `uuid`, so `(sessionId, uuid)` does NOT collapse duplicates — use it only as an identity for
records lacking `message.id`; inventory §8). **Subagent launch tool:** match both `Agent` and `Task`
(inventory §2.2). **Discovery:** search
...
3. **PR-3 (Tier 3):** Waste/efficiency detectors and asset-utilization findings feeding Collaboration
   Efficiency; delivery correlation feeding Delivery Predictability & Test Loop Completeness.
